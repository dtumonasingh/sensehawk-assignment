export default {
  ...
  addHighlight(state) {
    let payload = state.selectedWord;
        ...
        const highlightExists = state.highlights.some(highlight => highlight.value === payload.value);        
        if(highlightExists){            
            let index = state.highlights.findIndex(highlight => highlight.value === payload.value);
